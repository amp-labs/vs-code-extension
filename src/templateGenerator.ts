...
export async function generateTemplateForProvider(
...
`;
  ...
  const writeObjects = template.objects.write.filter((obj) => selectedReadObjects.includes(obj));
...
`;
  ...
  if (provider === 'salesforce' || provider === 'hubspot') {
    const subscribeObjects =
      template.objects.write.length > 0
        ? template.objects.write
        : Object.keys(template.objects.read);
