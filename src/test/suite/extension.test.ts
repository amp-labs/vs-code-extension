...
suite('Extension Test Suite', () => {
  ...
  test('Extension should be activated', async function () {
    ...
    const extension = vscode.extensions.getExtension(
      'undefined_publisher.ampersand-schema-validator'
    );
    ...
    console.log(
      ...
      allExtensions.map((ext) => ext.id)
    );

    const devExtension = allExtensions.find(
      (ext) => ext.id.toLowerCase().includes('ampersand') || ext.id.toLowerCase().includes('schema')
    );
